...
<%--상단 시작 --%>
			...
			<div id="cn">
				...
				<form action="/shoppage/shop" id="searchForm" method="get">
					<select name="type">
						...
						name="pageNum" value="${paging1.scri.amount}" hidden=""> <input
						type="text" name="amount" value="${paging1.scri.amount}" hidden="">
				...
				<div class="Goods">
					<ul class="Goodss">
						...
						<c:forEach items="${Snack}" var="Snack">
							<li>
								<div>
									<input type="text" value="${Snack.bno}" hidden="">
								</div>
								<div>
									<a href="/shoppage/Detail?bno=${Snack.bno}"> <img
										class="img" src="/display?filename=${Snack.filem}"></a>
								</div>
								<div>
									상품명<span class="font">${Snack.w_name}</span>
								</div>
								<div>
									가격<span class="font">${Snack.w_price}</span>
								</div>
								<div>
									<span>리뷰</span>
								</div>
	...
	<div id="pag">
		...
		<c:if test="${paging1.prev}">
			<a
				href="/shoppage/shop?pageNum=${paging1.startPage-1}&amount=${paging1.scri.amount}">이전</a>
		</c:if>

		<!-- begin(1) end(10)될 동안 반복(1일 10일 될 동안 반복) -->
		<c:forEach begin="${paging1.startPage}" end="${paging1.endPage}"
			var="num">
			<a
				href="/shoppage/shop?type=${paging1.scri.type}&keyword=${paging1.scri.keyword}&pageNum=${num}&amount=${paging1.scri.amount}">${num}</a>
		</c:forEach>

		<!-- next(다음)이 true이면 다음버튼 활성화 -->
		<c:if test="${paging1.next}">
			<a
				href="/shoppage/shop?pageNum=${paging1.endPage+1}&amount=${paging1.scri.amount}">다음</a>
		</c:if>
	</div>
	<br>
	<!-- main 끝 -->
