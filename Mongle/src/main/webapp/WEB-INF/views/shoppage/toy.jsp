				...
				<div class="Goods">
					<ul class="Goodss">
						...
						<c:forEach items="${toy}" var="toy">
							<li>
								<div>
									<input type="text" value="${toy.bno}" hidden="">
								</div>
								<div>
									<a href="/shoppage/Detail?bno=${toy.bno}"> <img
										class="img" src="/display?filename=${toy.filem}"></a>
								</div>
								<div>
									상품명<span class="font">${toy.w_name}</span>
								</div>
								<div>
									가격<span class="font">${toy.w_price}</span>
								</div>
								<div>
									<span>리뷰</span>
								</div>
